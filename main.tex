...
As an increasing number of online services become available and popular, the issue of how to share data between the services becomes prevalent. The data that they input and output can be in differing data formats and be difficult and time-consuming for a user to reconcile the results together. These are the primary issues that we are developing the Grassroots infrastructure to solve.
...
\row name & no & The user-friendly name of the parameter for displaying to a user. 
...
\row grassroots\_type\_text & yes & This can take one of the values in table  \ref{tab:parameter_types} \\
...
\item description: The user-friendly name of the parameter for displaying to a user. If this is not set, then the value for the *value* will be used instead.
\end{itemize}
An example of this is shown in Figure \ref{fig:parameter_enum} which indicates that the Parameter can take 1 of 3 possible values, "z",\textit{} "zip" or "gz", and the values to show 
to the user are "Use Raw", "Use Zip" and "Use GZip". \\
\row level & no & A number used to determine whether to show a Parameter to a user. The system offers 3 levels of Parameter; beginner, intermediate and advanced, and the user can switch their view as appropriate to show/hide different parameters. \\
\row level\_text & no & An alternative way of defining the levels available for a Parameter. The words "beginner", "intermediate" and "advanced" and "all" are available. \\
...
\begin{figure}[ht]
\centering
\includegraphics{Grassroots_architecture}
